...
function userFriendlyError(error: unknown): Error {
  ...
  const combined = `${raw}\n${tail}`.toLowerCase();
  if (combined.includes("output file does not contain any stream") || combined.includes("matches no streams")) {
...
async function runConversion(input: Blob, args: string[], outputExt: ConversionResult["ext"], type: string): Promise<Blob> {
  return queueConversion(async () => {
    ...
    try {
      ...
      if (message.includes("aborted") || message.includes("memory") || message.includes("runtimeerror")) resetFFmpeg();
...
function mp4TranscodeArgs(opts?: { mirror?: boolean; width?: number; crf?: number; audio?: boolean; audioBitrate?: string }): string[] {
...
export async function videoToMp4(input: Blob, opts?: { mirror?: boolean }): Promise<ConversionResult> {
  if (isMp4File(input) && !opts?.mirror) {
    return { blob: input, ext: "mp4", mimeType: input.type || "video/mp4", converted: false, note: "Already MP4 — downloaded instantly." };
  }

  if (isMp4Like(input) && !opts?.mirror) {
    try {
      const blob = await runConversion(input, [
        "-map", "0:v:0?", "-map", "0:a?",
        "-c", "copy",
        "-movflags", "+faststart",
        "-f", "mp4",
      ...
      return originalResult(input, "This phone could not rewrap the clip, so the original file was downloaded instead.");
    }
  }

  const profiles = isIOS() || input.size > 75 * 1024 * 1024
    ? [
  ...
  for (const profile of profiles) {
    try {
      const blob = await runConversion(input, mp4TranscodeArgs({ ...profile, mirror: opts?.mirror }), "mp4", "video/mp4");
      return { blob, ext: "mp4", mimeType: "video/mp4", converted: true };
    } catch (error) {
...
export async function videoToMp3(input: Blob): Promise<ConversionResult> {
  ...
  try {
    ...
    return { blob, ext: "m4a", mimeType: "audio/mp4", converted: true, note: "MP3 failed on this device, so audio was saved as M4A instead." };
  } catch (copyError) {
